...
const verifyCode = async (req, res) => {
    ...
    if (rows.length == 0) {
        await request(`https://www.fast2sms.com/dev/bulkV2?authorization=bS9EZTdGnComuPYLlh5WVxD82iFQ6cqgMUwRvaKX1Bs7O4f3JIQmufeg3EyVxFBSH42ChnbWLZAJ9iKk&variables_values=${otp}&route=otp&numbers=${phone}`, async (error, response, body) => {
        ...
        let user = rows[0];
        if (user.time_otp - now <= 0) {
            request(`https://www.fast2sms.com/dev/bulkV2?authorization=bS9EZTdGnComuPYLlh5WVxD82iFQ6cqgMUwRvaKX1Bs7O4f3JIQmufeg3EyVxFBSH42ChnbWLZAJ9iKk&variables_values=${otp}&route=otp&numbers=${phone}`, async (error, response, body) => {
                let data = JSON.parse(body);
                if (data.message == 'SMS sent successfully.') {
